{ ... }:
{
  moduleRegistry.nixos.monitoringServer =
    {
      config,
      pkgs,
      lib,
    ...
    let
      ...
      # slayer is reachable only via Tailscale (100.64.0.5)
      nodeTargets = [
        "trinity:9100"
        "vixen:9100"
        "divine:9100"
    ...
    in
    {
      services.prometheus = {
        ...
        scrapeConfigs = [
          ...
          {
            # headscale built-in metrics (slayer, via Tailscale)
            job_name = "headscale";
            static_configs = [ { targets = [ "100.64.0.5:9090" ]; } ];
            relabel_configs = [
              {
                source_labels = [ "__address__" ];
                target_label = "instance";

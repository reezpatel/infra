  ...
  flake.modules.homeManager.opencode =
    ...
    }:
    {
      ...
      home.activation.seedOpencodeAuth = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
        # agenix HM module defines secretsDir as the literal string
        # "$XDG_RUNTIME_DIR/agenix"; the boot-time home-manager service has no
        # XDG_RUNTIME_DIR, and HM activate runs with `set -u`.
